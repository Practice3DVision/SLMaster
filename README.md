...
## 依赖
**SLMaster**依赖的库包含如下几点：
- [OpenCV](https://github.com/opencv/opencv/tree/4.8.0) <img src="https://img.shields.io/badge/v4.8.0-passing-rgb(0, 255, 0)" />
...
## 4. 编译
当你获取到本库代码之后，首先检查上述依赖，若不满足依赖条件，可通过点击上述依赖库跳转至对应的库，随后下载其代码并进行编译。当完成所有的依赖库的编译之后，可开始着手编译**SLMaster**，首先使用**cmake-gui**选择到项目根路径，然后点击**configure**，若一切正常可看到以下选项：
![cmake-build](doc/cmake-build.png)
你可以勾选上`BUILD_TEST`，这将编译**google_test**中的测试用例，这些测试用例同样是一份非常不错的示例代码。
随后点击**generate**按钮生成对应的工程文件，最后，使用您喜爱的编译器对其进行编译即可。
## 5. 使用
