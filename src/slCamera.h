...
class SLMASTER_API SLCamera {
  public:
     ...
     * @return
     */
    virtual device::camera::CameraFactory* getCameraFactory() = 0;
    /**
     * @brief 获取投影仪工厂
     * @return
     */
    virtual device::projector::ProjectorFactory* getProjectorFactory() = 0;
    /**
     * @brief 设置条纹编解码方法
     ...
     * @return false 失败
     */
    virtual bool offlineCapture(IN const std::vector<std::vector<cv::Mat>>& imgs, OUT FrameData &frameData) = 0;
    /**
     * @brief 是否使能深度相机(@note
  ...
  protected:
    inline bool readJsonFile(const std::string jsonPath, Json::Value &jsonVal) {
        ...
        std::string errs;
        return Json::parseFromStream(readerBuilder, readStream, &jsonVal, &errs);
    }
