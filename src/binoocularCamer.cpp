...
bool BinocularCamera::loadParams(const std::string jsonPath,
    ...
    cv::initUndistortRectifyMap(caliInfo_->info_.M1_, caliInfo_->info_.D1_, caliInfo_->info_.R1_, caliInfo_->info_.P1_, imgSize, CV_32FC1, mapLX_, mapLY_);
    cv::initUndistortRectifyMap(caliInfo_->info_.M2_, caliInfo_->info_.D2_, caliInfo_->info_.R2_, caliInfo_->info_.P2_, imgSize, CV_32FC1, mapRX_, mapRY_);
...
bool BinocularCamera::saveParams(const std::string jsonPath,
                                 Json::Value &jsonVal) {
    if (jsonVal.empty()) {
        std::cerr << "binocular camera write json file error, json value is empty! \n" << std::endl;
        return false;
    }
...
SLCameraInfo BinocularCamera::getCameraInfo() {
    ...
    slCameraInfo.isFind_ = leftCameraInfo.isFind_ &&
                            rightCameraInfo.isFind_ && projectorInfo.isFind_;
...
void BinocularCamera::decode(const std::vector<std::vector<cv::Mat>> &imgs,
    ...
    cv::parallel_for_(cv::Range(0, imgs[0].size()), [&](const cv::Range& range) {
