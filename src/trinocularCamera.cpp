...
bool TrinocularCamera::saveParams(const std::string jsonPath,
                                 Json::Value &jsonVal) {
    if (jsonVal.empty()) {
        std::cerr << "trinocular camera write json file error, json value is empty! \n" << std::endl;
        return false;
    }
    ...
    try {
        ...
        const bool connectProjector =
            projectorFactory_.getProjector(stringProperties_["DLP Evm"])
                ->connect();
        connectState =
            connectLeftCamera && connectRightCamera && connectColorCamera && connectProjector;
        ...
        if (connectState) {
            ...
            cameraFactory_
                ...
                ->start();
            cameraFactory_
                .getCamera(stringProperties_["Color Camera Name"], manufator)->setEnumAttribute("BalanceRatioSelector", "Red");
            cameraFactory_
                .getCamera(stringProperties_["Color Camera Name"], manufator)->setNumberAttribute("Balance", 1.72477);
            cameraFactory_
                .getCamera(stringProperties_["Color Camera Name"], manufator)->setEnumAttribute("BalanceRatioSelector", "Green");
            cameraFactory_
                .getCamera(stringProperties_["Color Camera Name"], manufator)->setNumberAttribute("Balance", 1.0);
            cameraFactory_
                .getCamera(stringProperties_["Color Camera Name"], manufator)->setEnumAttribute("BalanceRatioSelector", "Blue");
            cameraFactory_
                .getCamera(stringProperties_["Color Camera Name"], manufator)->setNumberAttribute("Balance", 1.66687);
...
bool TrinocularCamera::isConnect() {
    ...
    return isConnectLeftCamera && isConnectRightCamera && isConnectColorCamera && isConnectProjector;
}

void TrinocularCamera::decode(const std::vector<std::vector<cv::Mat>> &imgs,
                             FrameData &frameData) {
    //默认深度相机为彩色相机，纹理相机为彩色相机
    //顺序依次为左相机、右相机、彩色相机(Camera 1、Camera 2、Camera 3)
    frameData.textureMap_ = cv::Mat::zeros(imgs[2][0].size(), imgs[2][0].type());
    ...
    pattern_->decode(imgsOperat, depthMap, booleanProperties_["Gpu Accelerate"]);
    ...
    fromDepthMapToCloud(depthMap, frameData.textureMap_, *caliInfo_, *frameData.pointCloud_, true);
}
...
bool TrinocularCamera::continuesCapture(SafeQueue<FrameData>& frameDataQueue) {
    ...
    imgCreateThread_ = std::thread([&]{
        ...
        while(!isCaptureStop_.load(std::memory_order_acquire)) {
            ...
            if(leftImgs.size() >= imgSizeWaitFor && rightImgs.size() >= imgSizeWaitFor && colorImgs.size() >= imgSizeWaitFor) {
...
bool TrinocularCamera::stopContinuesCapture() {
    ...
    cameraFactory_.getCamera(
                      stringProperties_["Color Camera Name"], manufator)->clearImgs();

    return true;
}

bool TrinocularCamera::offlineCapture(const std::vector<std::vector<cv::Mat>> &imgs,
                                     FrameData &frameData) {
...
bool TrinocularCamera::getStringAttribute(const std::string attributeName,
    ...
    if (!stringProperties_.count(attributeName)) {
        std::cerr << "property " << attributeName.data() << " is not be supported !" << std::endl;
        return false;
    }
...
bool TrinocularCamera::getNumbericalAttribute(const std::string attributeName,
    ...
    if (!numbericalProperties_.count(attributeName)) {
        std::cerr << "property " << attributeName.data() << " is not be supported !\n" << std::endl;
        return false;
    }
...
bool TrinocularCamera::resetCameraConfig() {
    ...
    stringProperties_["Manufactor"] = "@LiuYunhuang";
...
bool TrinocularCamera::burnPatterns(const std::vector<cv::Mat>& imgs) {
    ...
    for (size_t i = 0; i < patternSets.size(); ++i) {
        ...
        patternSets[i].imgs_ =
            6 * i + 6 < imgs.size()
                ? std::vector<cv::Mat>(imgs.begin() + 6 * i, imgs.begin() + 6 * (i + 1))
