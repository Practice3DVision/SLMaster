...
void twoDimensionErrorExpand(cv::Mat &img) {
    ...
    const float efficient[4] = {7.0f / 16.0f, 5.0f / 16.0f, 3.0f / 16.0f,
                                1.0f / 16.0f};
...
int opwmGetBinaryVal(const float wrappedPhaseVal, const bool inverse) {
    // const float truncatPhases[4] = { 0.173234670244565,
    // 0.447810198774037, 1.06049800055750, 1.08708997788648};
    const float truncatPhases[4] = {0.152789739321315, 0.490089110274083,
                                    1.05070711562070, 1.11882783612695};

    if ((wrappedPhaseVal >= truncatPhases[0] &&
         wrappedPhaseVal <= truncatPhases[1]) ||
        (wrappedPhaseVal >= truncatPhases[2] &&
         wrappedPhaseVal <= truncatPhases[3])) {
        return inverse ? 1 : 0;
    } else {
        return inverse ? 0 : 1;
    }
...
void opwm(cv::Mat &img, const int cycles, const float shiftVal,
    ...
    for (int j = 0; j < height; ++j) {
        ...
        for (int k = 0; k < width; ++k) {
            ...
            float wrappedPhaseVal = (k % pixelsPerPeriod) /
