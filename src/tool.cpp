...
namespace slmaster {
void fromDispairtyMapToCloud(const cv::Mat& disparityMap, const cv::Mat& textureMap, const CaliInfo& caliInfo, pcl::PointCloud<pcl::PointXYZRGB>& cloud, const bool isRemapColorCam) {
    ...
    cv::parallel_for_(cv::Range(0, rows), [&](const cv::Range& range) {
        for(int i = range.start; i< range.end; ++i) {
            ...
            for (int j = 0; j < cols; ++j) {
                ...
                pcl::PointXYZRGB cloudPoint(point(0, 0), point(1, 0), point(2, 0), 255, 255, 255);
...
void fromDepthMapToCloud(const cv::Mat& depthMap, const cv::Mat& textureMap, const CaliInfo& caliInfo, pcl::PointCloud<pcl::PointXYZRGB>& cloud, const bool isRemapColorCam) {
        ...
        for(int i = range.start; i< range.end; ++i) {
            ...
            for (int j = 0; j < cols; ++j) {
                ...
                if(isRemapColorCam) {
                    ...
                    const int xLoc = point(0, 0) / point(2, 0);
                    const int yLoc = point(1, 0) / point(2, 0);
                    if(xLoc >= 0 && xLoc < cols - 1 && yLoc >= 0 && yLoc < rows - 1) {
                        auto color = textureMap.ptr<cv::Vec3b>(yLoc)[xLoc];
                        cloudPoint.r = color[2];
                        cloudPoint.g = color[1];
                        cloudPoint.b = color[0];
