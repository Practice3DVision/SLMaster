...
class SLMASTER_API BinocularCamera : public SLCamera {
  public:
     ...
     * @return
     */
    device::CameraFactory *getCameraFactory() override {
        return &cameraFactory_;
    }
    /**
     * @brief 获取投影仪工厂
     * @return
     */
    device::ProjectorFactory *getProjectorFactory() override {
        return &projectorFactory_;
    }
    /**
     * @brief 烧录条纹
     * @param imgs 待烧录的条纹
     ...
     * @return false 失败
     */
    bool offlineCapture(IN const std::vector<std::vector<cv::Mat>> &leftImgs,
                        OUT FrameData &frameData) override;
    /**
     * @brief 是否使能深度相机
     *
